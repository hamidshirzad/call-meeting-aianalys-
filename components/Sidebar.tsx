...
const NavItem: React.FC<{
  ...
  onClick: () => void;
  disabled?: boolean;
}> = ({ feature, label, icon, isActive, onClick, disabled = false }) => (
  <li>
    <a
      href="#"
      onClick={(e) => {
    ...
    >
      ...
      {disabled && <span className="ml-auto text-xs font-bold bg-yellow-400 text-yellow-900 px-2 py-0.5 rounded-full">PRO</span>}
    </a>
  </li>
);
...
const PlanBadge: React.FC<{plan: SubscriptionPlan}> = ({ plan }) => {
    ...
    return (
        <span className={`ml-auto text-xs font-bold uppercase px-2 py-1 rounded-full ${planStyles[plan]}`}>
            {plan}
        </span>
    );
};
        ...
        <ul>
          {navItems.map((item) => (
            <NavItem
              ...
              disabled={(item.enterpriseOnly && userPlan !== 'enterprise')}
            />
          ))}
        </ul>
