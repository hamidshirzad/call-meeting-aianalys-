...
const ChatAssistant: React.FC<ChatAssistantProps> = ({ analysisContext, messages, setMessages, user, setActiveFeature }) => {
  ...
  return (
    <div className="h-full flex flex-col" style={{maxHeight: 'calc(100vh - 8rem)'}}>
          ...
          <AnimatePresence>
            {messages.map((msg, index) => (
              <motion.div
                ...
                className={`flex items-start gap-3 sm:gap-4 ${msg.role === 'user' ? 'justify-end' : ''}`}
              >
                {msg.role === 'model' && (
                  <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-indigo-500 to-emerald-500 flex-shrink-0 shadow-md"></div>
                )}
                <div
                  className={`max-w-lg p-3 sm:p-4 rounded-2xl shadow-md ${
                    msg.role === 'user'
        ...
        <div className="p-4 bg-slate-100 dark:bg-slate-900/50 border-t border-slate-200 dark:border-slate-700">
           <div className="flex items-center gap-2 mb-2 overflow-x-auto pb-2">
                {suggestionChips.map(suggestion => {
                  ...
                  return (
                    <button key={suggestion} onClick={() => handleSend(suggestion)} disabled={disabled} className="flex-shrink-0 px-3 py-1 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-full text-sm hover:bg-slate-300 dark:hover:bg-slate-600 transition disabled:opacity-50 disabled:cursor-not-allowed relative group">
                        {suggestion}
